...
class SparseVectorGenerator:
    ...
    def build_vocabulary(self, documents: list[str], incremental: bool = True) -> None:
        ...
        logger.info(
            f"Building vocabulary from {len(documents)} documents (incremental={incremental})..."
        )
    ...
    def save_stats(self, filepath: str) -> None:
        ...
        logger.info(
            f"Saved BM25 stats to {filepath}: {len(self.vocab)} tokens, {self.doc_count} docs"
        )
    ...
    def load_stats(self, filepath: str) -> None:
        ...
        logger.info(
            f"Loaded BM25 stats from {filepath}: {len(self.vocab)} tokens, {self.doc_count} docs"
        )
